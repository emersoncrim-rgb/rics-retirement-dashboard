...
use_live_quotes = st.sidebar.checkbox(
    "Enable live quote fetch (Finnhub)",
    value=False,
    help="If enabled, the app fetches current prices and can overlay them when 'Live overlay' is selected.",
)
...
def load_data(data_path: str, want_live: bool, api_key: str, refresh_nonce: int):
    ...
    # Disk ingest (baseline / last export)
    ingested_disk = ingest_all(paths)

    # Live ingest starts as a deep copy of disk ingest
    ...
    # Precompute summaries and reports for both modes
    ...
    return (
        ...
        quote_meta, quote_failures, price_overrides
    )
...
# Select which dataset drives calculations across tabs
use_live_for_calcs = (data_mode == "Live overlay (current quotes)")
if use_live_for_calcs and not quote_meta.get("enabled"):
    # Live selected but quotes not available; fall back silently and inform user.
    use_live_for_calcs = False
...
# Keep a pristine snapshot copy for comparison UI (disk baseline)
snapshot_accounts = copy.deepcopy(ingested_disk["accounts"])
...
# Live quotes status (sidebar)
st.sidebar.divider()
st.sidebar.subheader("Quote Status")
st.sidebar.caption(f"Mode in use: {'Live' if use_live_for_calcs else 'Disk'}")

if quote_meta.get("enabled"):
    st.sidebar.success(f"Live quotes applied: {quote_meta.get('updated',0)}/{quote_meta.get('fetched',0)} tickers updated")
    if quote_meta.get("asof_epoch"):
        st.sidebar.caption("As of: " + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(quote_meta["asof_epoch"])))
    with st.sidebar.expander("Coverage (details)", expanded=False):
    ...
    if use_live_quotes:
        st.sidebar.warning("Live quote fetch enabled but no prices were applied.")
        if quote_failures:
...
with tab_today:
    ...
    c4.metric("Score", f"{rr['aggressiveness_score']}/100", delta=rr["posture"]["label"])

    if quote_meta.get("enabled"):
        disk_total = float(summary_disk.get("total_portfolio", 0) or 0)
        live_total = float(summary_live.get("total_portfolio", 0) or 0)
        delta_val = live_total - disk_total
        pct_val = (delta_val / disk_total) if disk_total else 0.0
        st.caption(            f"Disk vs Live total value change: **{delta_val:+,.0f} ({pct_val:+.2%})** — switch Data Mode in the sidebar to control calculations."        )
...
if quote_meta.get("enabled"):
    ...
    for r in snapshot_accounts:
        ...
        if not t:
            continue
        snap_by_t.setdefault(t, {"snapshot_price": r.get("price"), "snapshot_value": 0.0})
        snap_by_t[t]["snapshot_value"] += float(r.get("market_value", 0) or 0)

    live_by_t = {}
    for r in ingested_live["accounts"]:
        t = (r.get("ticker") or "").strip()
        if not t:
            continue
        live_by_t.setdefault(t, {"live_price": r.get("price"), "live_value": 0.0})
    ...
    st.caption("Enable live quote fetch in the sidebar to compare snapshot vs current prices.")
...
with tab_determ:
    ...
    @st.cache_data
    def run_deterministic(data_path: str, h: int):
        paths = {k: str(Path(data_path) / v.split("/")[-1]) for k, v in DEFAULT_PATHS.items()}
        ing = ingest_all(paths)
    ...
    if st.button("Run Deterministic Projections", type="primary", key="run_det"):
        with st.spinner("Running 3-scenario projections..."):
            scenarios = run_deterministic(str(data_dir), horizon)
